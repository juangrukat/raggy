...
def _build_test_app():
    ...
    mock_connector.get_collection_names = AsyncMock(
        return_value=["docs", "hybrid_test"]
    )
    ...
    mock_provider_4b.get_vector_name = MagicMock(
        return_value="qwen3-qwen3-embedding-4b"
    )
    ...
    mock_provider_8b.get_vector_name = MagicMock(
        return_value="qwen3-qwen3-embedding-8b"
    )
    ...
    mock_mgr.create_provider_for_model = MagicMock(
        side_effect=_create_provider_for_model
    )
    ...
    with (
        patch("raggy_mcp.webui.api.QdrantConnector", return_value=mock_connector),
        patch(
            "raggy_mcp.webui.api.EnhancedEmbeddingModelManager", return_value=mock_mgr
        ),
...
def test_search_documents_modes_dense_hybrid_rerank(
    mode, expect_sparse, expect_reranker
):
